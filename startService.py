...
tableTitles = soup.find_all('div', {'class': 'MuiDataGrid-columnHeaderTitle css-cc8tf1'})
for tableTitle in tableTitles:
    print('-------------------------------[Header]')
    print (tableTitle.text)
for tableRow in soup.find_all('div', {'class': 'MuiDataGrid-cellContent'}):
    print('-------------------------------[Row]')
    print(tableRow.text)

for healthRecord in soup.find_all('path', attrs={'d': 'M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2z'}):
    print('-------------------------------[Health]')
    print('GOOD')
    print('the total records: ' + str(len(healthRecord)))
advRecords = []